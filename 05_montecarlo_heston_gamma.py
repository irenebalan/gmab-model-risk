...
OUTPUT_PATHS = (
    par.OUTPUT_DIR
    / "heston_model_risk_per_path_gamma.csv"
)

OUTPUT_SUMMARY = (
    par.OUTPUT_DIR
    / "heston_model_risk_sintesi_gamma.csv"
)

OUTPUT_PARAMETERS = (
    par.OUTPUT_DIR
    / "heston_parametri_stimati_gamma.csv"
)

OUTPUT_VARIANCE_REGRESSION = (
    par.OUTPUT_DIR
    / "heston_variance_regression_gamma.csv"
)

OUTPUT_GAMMA_REGRESSION = (
    par.OUTPUT_DIR
    / "heston_variance_gamma_regression.csv"
)

OUTPUT_PLOT = (
    par.OUTPUT_DIR
    / "heston_model_risk_rmse_gamma.png"
)

OUTPUT_DISTRIBUTION_PLOT = (
    par.OUTPUT_DIR
    / "heston_hedging_error_distribution_gamma.png"
)
...
def simulate_heston_paths(
    ...
    gamma_weighted_variance_numerator = np.zeros(
        N_PATHS,
        dtype=float,
    )

    gamma_weight_denominator = np.zeros(
        N_PATHS,
        dtype=float,
    )
    ...
    for step in range(
        ...
        gamma_weighted_variance_numerator += (
                gamma_weight
                * positive_variance
                * dt
        )

        gamma_weight_denominator += (
                gamma_weight
                * dt
        )
    ...
    gamma_weighted_realized_variance = np.divide(
        gamma_weighted_variance_numerator,
        gamma_weight_denominator,
        out=np.full(
            N_PATHS,
        ...
        where=gamma_weight_denominator > 0.0,
    )
    ...
    return (
        ...
        times,
        average_realized_variance,
        gamma_weighted_realized_variance,
    )
...
def heston_variance_gamma_regression(
    average_realized_variance,
    gamma_weighted_realized_variance,
    hedging_error,
):
    average_realized_variance = np.asarray(
        average_realized_variance,
        dtype=float,
    )

    gamma_weighted_realized_variance = np.asarray(
        gamma_weighted_realized_variance,
        dtype=float,
    )

    hedging_error = np.asarray(
        hedging_error,
        dtype=float,
    )

    valid = (
        np.isfinite(average_realized_variance)
        & np.isfinite(gamma_weighted_realized_variance)
        & np.isfinite(hedging_error)
    )

    x_variance = average_realized_variance[valid]
    x_gamma = gamma_weighted_realized_variance[valid]
    y = hedging_error[valid]

    if len(y) < 4:
        raise RuntimeError(
            "Osservazioni insufficienti per "
            "la regressione gamma-pesata."
        )
    ...
    coefficients_base = np.linalg.lstsq(
        X_base,
        y,
        rcond=None,
    )[0]
    ...
    coefficients_full = np.linalg.lstsq(
        X_full,
        y,
        rcond=None,
    )[0]
    ...
    return pd.DataFrame(
        [
            {
                "n_percorsi": len(y),
                "alpha": coefficients_full[0],
                "beta_varianza_media": coefficients_full[1],
                "beta_varianza_gamma_pesata": coefficients_full[2],
                "r_squared_base": r_squared_base,
                "r_squared_full": r_squared_full,
                "incremento_r_squared": (
    r_squared_full
    - r_squared_base
),
"partial_r_squared_gamma": (
...
"correlazione_regressori": (
    correlation_regressors
),
                "varianza_media_realizzata_media": (
                    x_variance.mean()
                ),
                "varianza_gamma_pesata_media": (
                    x_gamma.mean()
                ),
            }
        ]
...
def main():
        ...
        [
            {
                ...
                "pricing_simulazione": (
                    f"misura Q con lambda varianza = "
    ...
    if np.isclose(
        ...
        print(
            "Scenario di sensitivita': lambda diverso "
            "da zero; kappa_Q e theta_Q sono ottenuti "
            "applicando la trasformazione P -> Q."
        )
    ...
    gamma_regression = (
        heston_variance_gamma_regression(
            average_realized_variance,
            gamma_weighted_realized_variance,
            heston_error,
        )
    ...
    gamma_regression.to_csv(
        OUTPUT_GAMMA_REGRESSION,
        index=False,
    )

    print(
        "\n=== HESTON: REGRESSIONE CON VARIANZA GAMMA-PESATA ==="
    )

    print(
        gamma_regression.to_string(
            index=False,
        )
    )
